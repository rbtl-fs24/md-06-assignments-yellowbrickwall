...
2.  What's the `tidyverse` Package? Describe in maximum two sentences below.
    -   A collection of functions which allow for faster data exploration and transformation than if you only used the base R language
...
2.  Execute the code and observe the output in the Console. Would you publish this table in a report in this format?
    ...
    -   I would fix the look of it of course, but it's much better than the other option.
...
ggplot(data = waste_data_long_mean, 
  ...
  scale_fill_manual(values = c("lightblue", "blue", "green","darkgreen")) +
  theme_minimal()
